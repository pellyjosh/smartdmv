import { dbTable, text, timestamp, integer, boolean, primaryKeyId } from '@/db/db.config';
...
export const medicalImaging = dbTable("medical_imaging", {
  ...
  studyDate: timestamp('study_date', { mode: 'date' }).notNull(),
  ...
  status: text("status", { enum: medicalImagingStatusEnum }).notNull().default(sql`'pending'`),
...
export const imagingAnnotations = dbTable("imaging_annotations", {
  ...
  color: text("color").default("#FF0000" as any), // Color of annotation
...
export const electronicSignatures = dbTable("electronic_signatures", {
  ...
  signedAt: timestamp('signed_at', { mode: 'date' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
  createdAt: timestamp('created_at', { mode: 'date' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
  metadata: text("metadata").default(sql`'{}'`), // Additional data like consent text version (JSON string)
