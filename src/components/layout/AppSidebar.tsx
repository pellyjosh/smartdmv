...
export function AppSidebar({ isCollapsed, onToggleCollapse }: AppSidebarProps) {
  ...
  React.useEffect(() => {
    ...
    filteredNavItems.forEach(item => {
      if (item.submenu && item.submenu.length > 0) {
        const isParentActive = item.href ? (item.href === '/' ? pathname === '/' : pathname.startsWith(item.href)) : false;
  ...
  }, [filteredNavItems, pathname, toggleMenu, expandedMenus, user?.role, router, searchTerm]); // Add searchTerm to deps
