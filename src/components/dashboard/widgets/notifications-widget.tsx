...
import { Loader2, Bell, CheckCircle, AlertCircle, Info, MessageSquare } from "lucide-react";
...
interface DashboardNotification {
  ...
  title: string;
  message: string;
  type: 'appointment' | 'healthPlan' | 'message' | 'system' | 'info' | 'alert' | 'reminder';
  read: boolean;
  link?: string | null;
...
export function NotificationsWidget({ widget }: NotificationsWidgetProps) {
  ...
  const { data: notifications, isLoading, error } = useQuery<DashboardNotification[]>({
    queryKey: ['dashboard-notifications'],
    queryFn: async () => {
      const response = await fetch('/api/dashboard/notifications?limit=5&includeRead=false');
      if (!response.ok) {
    ...
    <div className="space-y-3">
      ...
      {notifications.some(n => !n.read) && (
        <div className="pt-2 border-t">
          ...
          >
            {markAsReadMutation.isPending ? 'Marking as read...' : 'Mark all as read'}
          </Button>
        </div>
      )}
    </div>
