...
const WIDGET_CATALOG: Record<string, WidgetDefinition> = {
  ...
  healthPlans: {
    ...
    title: "Health Plans",
    description:
      "Upcoming and active patient health plans and wellness programs",
    icon: (
  ...
  petStats: {
    ...
    description:
      "Statistical overview of patients, species distribution, and check-ins",
    icon: (
  ...
  const handleDeleteClick = (e: React.MouseEvent) => {
    ...
    if (isDefault) {
      toast({
        ...
        description:
          "Please set another dashboard as default before deleting this one",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleSetDefaultClick = (e: React.MouseEvent) => {
    ...
    if (!selectedConfigId) {
      toast({
        title: "No dashboard selected",
        description:
          "Please select a dashboard configuration to set as default",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="flex items-center gap-2 mb-4">
      ...
      {onDeleteConfig && (
        <Button
          ...
          disabled={
            !selectedConfigId || configs.length <= 1 || isSelectedDefault
          }
        >
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this dashboard configuration? This
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to set this dashboard as the default? This
              will make it the default dashboard for your practice.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
...
const AddWidgetDialog: React.FC<AddWidgetDialogProps> = ({
  ...
  const { user } = useUser();
  const practiceIdForRoles = Number(
    (user &&
      ("practiceId" in user
        ? (user as any).practiceId
        : "currentPracticeId" in user
  ...
  const userHasAnyRoleLocal = (allowed: string[]) => {
    ...
    if (isSuperAdminAssigned(assignedRoles) && allowed.includes("SUPER_ADMIN"))
      return true;
    if (
      isPracticeAdminAssigned(assignedRoles) &&
      (allowed.includes("PRACTICE_ADMIN") ||
        allowed.includes("PRACTICE_ADMINISTRATOR"))
    )
      return true;
    if (!Array.isArray(assignedRoles) || assignedRoles.length === 0)
      return false;
    return allowed.some((ar) =>
      assignedRoles.some((r: any) => {
  ...
  const availableWidgets = Object.values(WIDGET_CATALOG).filter((widget) =>
    userHasAnyRoleLocal(widget.allowedRoles)
  );
...
export function CustomizableDashboard() {
  ...
  const [currentConfig, setCurrentConfig] =
    useState<DashboardConfigData | null>(null);
  ...
  useEffect(() => {
    if (defaultConfig && !selectedConfigId) {
      ...
      try {
        const parsedConfig =
          typeof defaultConfig.config === "string"
  ...
  const handleSaveLayout = () => {
    ...
    (updateConfig as any)(
      ...
      },
      {
        ...
        onError: () => {
          // keep hasChanges so user can retry
        },
      }
    );
  };
  ...
  const handleCreateDashboard = (name: string, isDefault: boolean) => {
    ...
    (createConfig as any)(
      ...
      },
      {
        ...
        onError: () => {
          // leave dialog open so user sees error toast
        },
      }
    );
  };
  ...
  const handleDeleteConfig = (configId: number) => {
    ...
    if (selectedConfigId === configId) {
      ...
      if (otherConfigs.length > 0) {
        ...
        try {
          const parsedConfig =
            typeof nextConfig.config === "string"
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col items-center h-[calc(100vh-250px)] text-center">
          ...
          <p className="text-muted-foreground mb-6 max-w-md">
            Add widgets to customize your dashboard and get a quick overview of
            your practice.
          </p>
