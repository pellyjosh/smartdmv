...
import { MoreHorizontal, FilePenLine, Trash2, SquareCheck, Calendar, User, Eye } from 'lucide-react';
...
export default function AssignedChecklistsList() {
  ...
  if (checklists.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">No treatment checklists found. Create one to get started.</p>
      </div>
    );
  }
  ...
  return (
    <div>
            ...
            <TableRow key={checklist.id}>
              <TableCell>
                <div className="flex items-center space-x-2">
                  <Avatar className="h-8 w-8" style={{ backgroundColor: getAvatarColorFromName(checklist.petName) }}>
                    <AvatarFallback>{getInitials(checklist.petName)}</AvatarFallback>
                  </Avatar>
                  <div>
                    <div className="font-medium">{checklist.petName}</div>
                    <div className="text-xs text-muted-foreground">{checklist.petSpecies}</div>
                  </div>
                </div>
              </TableCell>
              <TableCell className="font-medium">{checklist.name}</TableCell>
              <TableCell>{getStatusBadge(checklist.status, checklist.dueDate)}</TableCell>
              <TableCell>{getPriorityBadge(checklist.priority || 'medium')}</TableCell>
              <TableCell>
                <div className="flex flex-col gap-1">
                  <Progress value={getProgressPercentage(checklist)} className="h-2" />
                  <span className="text-xs text-muted-foreground">
                    {`${checklist.items?.filter((i: any) => i.completed).length || 0}/${checklist.items?.length || 0} tasks`}
                  </span>
                </div>
              </TableCell>
              <TableCell>
              ...
              <TableCell className="text-right">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => setChecklistToView(checklist)}>
                      <Eye className="mr-2 h-4 w-4" />
                      View Details
                    </DropdownMenuItem>
                    {canManageChecklists && (
                      <DropdownMenuItem onClick={() => setChecklistToEdit(checklist)}>
                        <FilePenLine className="mr-2 h-4 w-4" />
                        Edit Checklist
                      </DropdownMenuItem>
                    )}
                    <DropdownMenuItem onClick={() => setChecklistToComplete(checklist)}>
      ...
      <AlertDialog open={!!checklistToComplete} onOpenChange={(open) => !open && setChecklistToComplete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Complete Checklist?</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to mark this entire checklist as completed?
              {checklistToComplete && getProgressPercentage(checklistToComplete) < 100 && (
                <span className="block mt-2 text-amber-500 font-medium">
                  Warning: Not all tasks in this checklist have been completed.
                </span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
