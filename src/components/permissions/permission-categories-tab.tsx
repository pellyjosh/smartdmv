...
export default function PermissionCategoriesTab({ practiceId, isSuperAdmin }: PermissionCategoriesTabProps) {
  ...
  const [isGeneratePermissionsDialogOpen, setIsGeneratePermissionsDialogOpen] = useState(false);
  ...
  const { data: permissionCategories, isLoading: isLoadingCategories } = useQuery<PermissionCategory[]>({
  ...
  const { data: resourceCategoryMappings, isLoading: isLoadingMappings } = useQuery<ResourceCategoryMapping[]>({
    queryKey: ['/api/permission-categories', 'mappings', { practiceId }],
    queryFn: async () => {
      const response = await fetch(`/api/permission-categories?practiceId=${practiceId}`);
      if (!response.ok) throw new Error('Failed to fetch resource category mappings');
  ...
  const createPermissionCategoryMutation = useMutation({
    mutationFn: async (data: Omit<PermissionCategory, 'id' | 'createdAt' | 'updatedAt'>) => {
      const response = await apiRequest('POST', '/api/permission-categories', data);
      return await response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/permission-categories', { practiceId }] });
      setNewCategory({
        name: '',
        description: '',
        displayOrder: 0
  ...
  const updatePermissionCategoryMutation = useMutation({
    mutationFn: async (data: PermissionCategory) => {
      const response = await apiRequest('PATCH', `/api/permission-categories/${data.id}`, data);
      return await response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/permission-categories', { practiceId }] });
  ...
  const deletePermissionCategoryMutation = useMutation({
    mutationFn: async (categoryId: number) => {
      const response = await apiRequest('DELETE', `/api/permission-categories/${categoryId}`);
  ...
  const createResourceCategoryMappingMutation = useMutation({
    mutationFn: async (data: Omit<ResourceCategoryMapping, 'id' | 'createdAt' | 'updatedAt'>) => {
      // Mock implementation since the API endpoint doesn't exist
      console.log('Creating resource category mapping:', data);
      return { id: Date.now(), ...data };
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/permission-categories', 'mappings', { practiceId }] });
      setNewMapping({
        resourceType: '',
        categoryId: 0
      });
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Manage permission categories to organize resources into logical groups
            </CardDescription>
          </div>
          <Button
        ...
        <CardContent>
          {isLoadingCategories ? (
            ...
            <Table>
              ...
              <TableBody>
                {permissionCategories.map((category) => {
                  const categoryMappings = resourceCategoryMappings?.filter(
                        ...
                        <div className="flex flex-wrap gap-1">
                          {categoryMappings.length > 0 ? (
                            categoryMappings.map(mapping => (
                              <Badge key={mapping.id} variant="outline" className="mr-1">
                                {getResourceTypeName(mapping.resourceType as ResourceType)}
                              </Badge>
                            ))
                          ) : (
            ...
            <div className="text-center p-8 border rounded-md bg-muted/10">
              <h3 className="text-lg font-medium mb-2">No permission categories found</h3>
              <p className="text-muted-foreground mb-4">
                Create your first permission category to start organizing resources.
              </p>
              <Button 
                onClick={() => setIsAddCategoryDialogOpen(true)}
              >
      ...
      <Card>
        ...
        <CardContent>
          {isLoadingMappings ? (
          ...
          ) : resourceCategoryMappings && resourceCategoryMappings.length > 0 ? (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {resourceCategoryMappings.map(mapping => {
                  const category = permissionCategories?.find(c => c.id === mapping.categoryId);
                  return (
                    <TableRow key={mapping.id}>
                      <TableCell>{getResourceTypeName(mapping.resourceType as ResourceType)}</TableCell>
                      <TableCell>{category?.name || `Category ID: ${mapping.categoryId}`}</TableCell>
                      <TableCell className="text-right">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => {
                            deleteResourceCategoryMappingMutation.mutate(mapping.id);
                          }}
                        >
                          <Trash className="h-3.5 w-3.5" />
                          <span className="sr-only">Delete</span>
            ...
            <div className="text-center p-8 border rounded-md bg-muted/10">
              <h3 className="text-lg font-medium mb-2">No resource-category mappings found</h3>
              <p className="text-muted-foreground mb-4">
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setNewCategory({...newCategory, displayOrder: parseInt(e.target.value, 10) || 0})}
                placeholder="1"
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={!newCategory.name || createPermissionCategoryMutation.isPending}
            >
        ...
        <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setEditingCategory({...editingCategory, displayOrder: parseInt(e.target.value, 10) || 0})}
                />
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={!editingCategory?.name || updatePermissionCategoryMutation.isPending}
            >
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(value) => setNewMapping({...newMapping, resourceType: value})}
              >
            ...
            {!selectedCategoryId && (
              <div className="grid gap-2">
                ...
                <Select
                  value={newMapping.categoryId ? newMapping.categoryId.toString() : ''}
                  onValueChange={(value) => setNewMapping({...newMapping, categoryId: parseInt(value, 10)})}
                >
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={!newMapping.resourceType || !newMapping.categoryId || createResourceCategoryMappingMutation.isPending}
            >
