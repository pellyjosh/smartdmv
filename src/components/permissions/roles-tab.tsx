...
const RolesTab = ({ practiceId, isSuperAdmin }: RolesTabProps) => {
  ...
  const [expandedRoleId, setExpandedRoleId] = useState<number | string | null>(
    null
  );
  ...
  const [localPermissionStates, setLocalPermissionStates] = useState<{
    [key: string]: boolean;
  }>(() => {
  ...
  const { data: rolePermissions = [], isLoading: isLoadingPermissions } =
    useQuery<any[]>({
      ...
      queryFn: async () => {
        ...
        const response = await fetch(
          `/api/role-permissions?roleId=${expandedRoleId}`
        );
    ...
    mutationFn: async ({
      ...
      try {
        ...
        if (permData) {
          ...
          const isCustomRole =
            typeof roleId === "number" || !isNaN(Number(roleId));
          const customRoleId = isCustomRole ? Number(roleId) : null;

          console.log(
            `Updating permission for ${
              isCustomRole ? "custom" : "standard"
            } role. Role ID: ${roleId}, Custom Role ID: ${customRoleId}`
          );

          // Try to update a single permission first
          console.log(
            `Updating single permission ${permissionId} for role ${roleId} to granted=${granted}`
          );
          const directResponse = await apiRequest(
            "PATCH",
            `/api/role-permissions/${permissionId}`,
            {
              granted,
              roleId,
      ...
      const preparedPermissions = Array.isArray(permissions)
        ? permissions.map((p) => {
      ...
      if (
        permissionId &&
        !preparedPermissions.some((p) => p.id === permissionId)
      ) {
    ...
    onSuccess: (_data, variables: any) => {
      ...
      if (variables?.roleId) {
        queryClient.invalidateQueries({
          queryKey: ["/api/role-permissions", variables.roleId],
        });
        queryClient.invalidateQueries({
          queryKey: [`/api/roles/${variables.roleId}/permissions`],
        });
      }
    },
    ...
    <>
                                    ...
                                    {isLoadingPermissions ? (
                                      ...
                                      <ScrollArea className="h-[400px] pr-4">
                                        ...
                                        <div className="space-y-6">
                                          {(() => {
                                            console.log(
                                              `Roles tab - Rendering permissions for role: ${expandedRoleId}`
                                            );
                                            console.log(
                                              `Roles tab - rolePermissions length: ${
                                            ...
                                            ([
                                              ...
                                              const toggleAllPermissions = (
                                                newGrantedState: boolean
                                              ) => {
                                                categoryPermissions.forEach(
                                                  (permission: any) => {
                                                    // Find if this permission is granted in rolePermissions
                                                    const permissionInRole =
                                                      rolePermissions?.find(
                                                      ...
                                                      return resourceTypes.map(
                                                        (
                                                          ...
                                                          return (
                                                            <div
                                                              key={resourceType}
                                                              className="mb-4 bg-muted/30 rounded-md p-3"
                                                            >
                                                                      ...
                                                                      >
                                                                        <div className="flex-1">
                                                                          <h5 className="text-sm font-medium flex items-center">
                                                                            {hasLocalState && (
                                                                              <span
                                                                                className="w-2 h-2 rounded-full bg-yellow-500 mr-1.5 inline-block"
                                                                          ...
                                                                          <p className="text-xs text-muted-foreground">
                                                                            {permission.description ||
                                                                              `Can ${permission.action.toLowerCase()} ${permission.resourceType.toLowerCase()}`}
                                                                          </p>
                                                                        </div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <Form {...editRoleForm}>
            <form
              onSubmit={editRoleForm.handleSubmit(onEditSubmit)}
              className="space-y-4 py-4"
            >
              <FormField
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <Form {...createRoleForm}>
            <form
              onSubmit={createRoleForm.handleSubmit(onCreateSubmit)}
              className="space-y-4 py-4"
            >
              <FormField
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4">
            <p className="text-sm text-muted-foreground">
              Users assigned to this role will need to be reassigned to a
              different role.
            </p>
          </div>
          <DialogFooter>
