...
export default function PetSOAPNotesPage() {
  ...
  const { data: soapNotes, isLoading: isNotesLoading, error: notesError } = useQuery({
    queryKey: ['soapNotes', 'pet', petId],
    queryFn: async () => {
      const response = await apiRequest("GET", `/api/soap-notes?petId=${petId}`);
  ...
  const filteredNotes = soapNotes?.filter((note: SOAPNote) => {
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">{pet.name}'s Medical Records</h1>
              <p className="text-muted-foreground">
      ...
      <div className="flex items-center space-x-4">
        <div className="flex items-center space-x-2">
          ...
          <Button
            variant={selectedFilter === 'by-practitioner' ? 'default' : 'outline'}
            size="sm"
      ...
      {isNotesLoading ? (
        ...
        <div className="space-y-4">
          {filteredNotes
            .sort((a: SOAPNote, b: SOAPNote) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
            .map((note: SOAPNote) => (
            <Card key={note.id}>
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    <CardDescription>
                      Created by Practitioner ID: {note.practitionerId} at {format(new Date(note.createdAt), 'p')}
                    </CardDescription>
                  </div>
                  <div className="flex items-center space-x-2">
