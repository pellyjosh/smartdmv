...
import { Loader2, ArrowLeft, Edit, Calendar, User, MapPin, Pill, AlertTriangle, FileText } from "lucide-react";
...
const VaccinationDetailsPage = () => {
  ...
  const { data: vaccination, isLoading, error } = useQuery({
  ...
  const getStatusBadge = (vaccination: any) => {
    ...
    if (nextDueDate) {
      ...
      if (dueDate < thirtyDaysFromNow) {
        return <Badge variant="outline" className="border-yellow-500 text-yellow-700">Booster Due Soon</Badge>;
      }
    }
    ...
    return (
      <div className="container mx-auto py-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-600 mb-2">Vaccination Not Found</h1>
          <p className="text-muted-foreground mb-4">
            The vaccination record you're looking for doesn't exist or you don't have permission to view it.
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <p>
                  {vaccination.pet?.dateOfBirth 
                    ? `${Math.floor((Date.now() - new Date(vaccination.pet.dateOfBirth).getTime()) / (365.25 * 24 * 60 * 60 * 1000))} years`
                    : "Unknown"
                  }
                </p>
              </div>
            ...
            {vaccination.pet?.owner && (
              <>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{vaccination.pet.owner.email}</p>
                  <p className="text-sm text-muted-foreground">{vaccination.pet.owner.phone}</p>
                </div>
              </>
            )}
          </CardContent>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">{vaccination.vaccineName}</p>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Administration Date</label>
                  <p className="flex items-center gap-2">
                    <Calendar className="h-4 w-4" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="capitalize">{vaccination.route || "Not specified"}</p>
                  </div>
                  <div>
                ...
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Administration Site</label>
                  <p className="flex items-center gap-2">
            ...
            <div>
              <label className="text-sm font-medium text-muted-foreground">Administering Veterinarian</label>
              <p className="font-medium">
                {vaccination.administeringVet?.name || "Not specified"}
              </p>
              {vaccination.administeringVet?.email && (
                <p className="text-sm text-muted-foreground">{vaccination.administeringVet.email}</p>
              )}
            </div>
      ...
      {vaccination.vaccineType && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Details about this vaccine type from the practice's vaccine library
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <p className="font-medium capitalize">{vaccination.vaccineType.type}</p>
              </div>
              ...
              <div>
                <label className="text-sm font-medium text-muted-foreground">Duration of Immunity</label>
                <p>{vaccination.vaccineType.durationOfImmunity || "Not specified"}</p>
              </div>
            </div>

            {vaccination.vaccineType.diseasesProtected && (
              <div>
                <label className="text-sm font-medium text-muted-foreground">Diseases Protected Against</label>
                <p className="mt-1">{vaccination.vaccineType.diseasesProtected}</p>
              </div>
            )}
            ...
            {vaccination.vaccineType.sideEffects && (
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">{vaccination.vaccineType.sideEffects}</p>
              </div>
            )}
            ...
            {vaccination.vaccineType.contraindications && (
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">{vaccination.vaccineType.contraindications}</p>
              </div>
            )}
          </CardContent>
        </Card>
