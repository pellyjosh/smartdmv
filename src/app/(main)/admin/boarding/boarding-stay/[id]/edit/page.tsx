...
export default function BoardingReservationPage() {
  ...
  const res = await apiRequest("PUT", `/api/boarding/stays/${stayId}`, payload);
      if (!res.ok) {
  ...
  const mockPets: Pet[] = [
    { id: 1, name: "Buddy", ownerId: 1, ownerName: "John Doe", species: "Dog", breed: "Labrador" },
    { id: 2, name: "Max", ownerId: 2, ownerName: "Sarah Smith", species: "Dog", breed: "German Shepherd" },
    { id: 3, name: "Charlie", ownerId: 3, ownerName: "Emily Johnson", species: "Dog", breed: "Golden Retriever" },
    { id: 4, name: "Luna", ownerId: 4, ownerName: "Mark Wilson", species: "Cat", breed: "Siamese" }
  ];
  
  const mockKennels: Kennel[] = [
    { id: 1, name: "Kennel A1", type: "standard", size: "medium", isActive: true },
    { id: 2, name: "Kennel B2", type: "deluxe", size: "large", isActive: true },
    { id: 3, name: "Kennel C3", type: "premium", size: "large", isActive: true },
    { id: 4, name: "Kennel D4", type: "cats_only", size: "small", isActive: true },
    { id: 5, name: "Kennel E5", type: "isolation", size: "medium", isActive: true }
  ];
  
  // Use mock data until API is connected
  const displayPets: Pet[] = (pets as Pet[] | undefined) || mockPets;
  const displayKennels: Kennel[] = (kennels as Kennel[] | undefined) || mockKennels;
  ...
  const onSubmit = (data: z.infer<typeof formSchema>) => {
    ...
    let hasError = false;
    if (!petIdNum || isNaN(petIdNum)) {
      form.setError("petId", { type: "manual", message: "Please select a pet" });
      hasError = true;
    }
    if (!kennelIdNum || isNaN(kennelIdNum)) {
      form.setError("kennelId", { type: "manual", message: "Please select a kennel" });
      hasError = true;
    }
    if (!start) {
      form.setError("startDate", { type: "manual", message: "Invalid start date" });
      hasError = true;
    }
    if (!end) {
                        ...
                        <SelectContent>
                          {displayPets.map((pet) => (
                            <SelectItem key={pet.id} value={String(pet.id)}>
                              {pet.name} ({pet.ownerName}) - {pet.species}{pet.breed ? `, ${pet.breed}` : ''}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                        ...
                        <PopoverTrigger asChild>
                          <FormControl>
                            ...
                            >
                              ...
                              {toDate(field.value) ? format(toDate(field.value) as Date, "PP") : "Select date"}
                            </Button>
                          </FormControl>
                        </PopoverTrigger>
              ...
              {isEditMode ? (
                <>
                  ...
                  <div className="flex gap-2">
                    <Link href={`/admin/boarding/boarding-stay/${String(params?.id)}`}>
