...
export async function GET(request: NextRequest) {
  ...
  const petRows = await db.select().from(pets).where(and(eq(pets.ownerId, clientId), eq(pets.practiceId, Number(ctx.practiceId))));
  const petIds = petRows.map(p => p.id as number);

  const lists = petIds.length ? await db.select().from(assignedChecklists)
  ...
  const items = checklistIds.length ? await db.select().from(checklistItems).where(inArray(checklistItems.checklistId as any, checklistIds as any)) : [];
