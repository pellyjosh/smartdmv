...
const setClientCookie = (
  ...
  console.log(
    `[UserContext setClientCookie] Cookie ${name} ${value ? "set" : "deleted"}.`
  );
};
...
export function UserProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (cachedUserData) {
        ...
        if (userData && userData.id) {
          setUser(userData);
          console.log(
            "[UserContext INIT_CACHE] Loaded cached user data on initialization:",
            userData.email,
            userData.role
          );
        }
      ...
      console.error(
        "[UserContext INIT_CACHE_ERROR] Error loading cached user data on initialization:",
        error
      );
    }
  }, []);
  ...
  const fetchUser = useCallback(async () => {
    console.log(
      "[UserContext fetchUser START] Attempting to fetch current user from /api/auth/me. Current path:",
      pathname
    );
    setIsLoading(true);
    ...
    try {
      if (result.data === null) {
        ...
        console.log(
          "[UserContext fetchUser NO_SESSION] /api/auth/me returned null (no session). User cleared."
        );
      } else if (result.data) {
        ...
        if (userData && userData.id) {
          ...
          try {
            ...
            sessionStorage.setItem(
              SESSION_TOKEN_COOKIE_NAME,
              JSON.stringify(anyUser)
            );
            setClientCookie(SESSION_TOKEN_COOKIE_NAME, JSON.stringify(anyUser));
            console.log(
              "[UserContext fetchUser SUCCESS] User fetched and set:",
              anyUser.email,
              anyUser.role
            );
          } catch (mapErr) {
            ...
            console.log(
              "[UserContext fetchUser SUCCESS_NO_MAP] User fetched and set (no mapping):",
              userData.email,
          ...
          setClientCookie(SESSION_TOKEN_COOKIE_NAME, null);
          console.log(
            "[UserContext fetchUser NO_USER] /api/auth/me returned no user or invalid data."
          );
        }
        ...
        if (result.isNetworkError || result.isDatabaseError) {
          console.warn(
            "[UserContext fetchUser NETWORK_ERROR] Network/database error occurred:",
            result.error
          );
          ...
          try {
            ...
            if (cachedUserData) {
              ...
              if (userData && userData.id) {
                setUser(userData);
                console.log(
                  "[UserContext fetchUser CACHED] Loaded cached user data during network error:",
                  userData.email,
                  userData.role
                );
              }
              ...
              console.log(
                "[UserContext fetchUser CACHED] No cached user data found during network error"
              );
            }
          } catch (cacheError) {
            console.error(
              "[UserContext fetchUser CACHED_ERROR] Error loading cached user data:",
              cacheError
            );
          }
          ...
          setClientCookie(SESSION_TOKEN_COOKIE_NAME, null);
          console.warn(
            "[UserContext fetchUser API_FAIL] Auth API call failed:",
            result.error
          );
        }
      }
      ...
      console.error(
        "[UserContext fetchUser CATCH_ERROR] Error processing auth response:",
        error
      );
      setUser(null);
      sessionStorage.removeItem(SESSION_TOKEN_COOKIE_NAME);
      ...
      console.log(
        "[UserContext fetchUser FINALLY] fetchUser finished. isLoading:",
        false,
        "initialAuthChecked:",
        true,
        "User set to:",
  ...
  useEffect(() => {
    console.log(
      "[UserContext Mount/Effect] Initializing user state. Calling fetchUser."
    );
    fetchUser();
  }, [fetchUser]);
  ...
  useEffect(() => {
    const handleOnline = () => {
      console.log(
        "[UserContext Network] Connection restored, retrying auth check..."
      );
      if (networkError) {
        fetchUser();
      }
  ...
  const navigateBasedOnRole = useCallback(
    (role: User["role"]) => {
      console.log(
        `[UserContext Nav] Navigating based on role: ${role}. Current pathname: ${pathname}`
      );
      ...
      switch (role) {
        ...
        default:
          console.warn(
            `[UserContext Nav] Unknown role for navigation: ${role}. Redirecting to access denied.`
          );
          targetPath = "/access-denied";
      }
  ...
  useEffect(() => {
    console.log(
      "[UserContext Nav Effect Check] user:",
      user ? user.email : null,
      "initialAuthChecked:",
      initialAuthChecked,
      "pathname:",
    ...
    if (!isLoading && initialAuthChecked) {
      if (user && pathname === AUTH_PAGE) {
        console.log(
          "[UserContext Nav Effect EXECUTE] User authenticated and on login page. Redirecting. Role:",
          user.role
        );
        ...
        console.log(
          `[UserContext Nav Effect EXECUTE] User not authenticated but on a protected page (${pathname}). Redirecting to login.`
        );
        router.push(AUTH_PAGE);
      }
    } else {
      console.log(
        "[UserContext Nav Effect] Conditions for navigation not met (still loading/checking or path is public/login with no user)."
      );
    }
  }, [
    user,
  ...
  const login = async (
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          "[UserContext login] API login failed:",
      ...
      console.warn(
        "[UserContext login] API returned ok, but no valid user data."
      );
      setUser(null);
      sessionStorage.removeItem(SESSION_TOKEN_COOKIE_NAME);
      setClientCookie(SESSION_TOKEN_COOKIE_NAME, null);
      ...
      setInitialAuthChecked(true);
      console.log(
        "[UserContext login FINALLY] isLoading: false, initialAuthChecked: true"
      );
    }
  };
  ...
  const logout = async () => {
    ...
    try {
      ...
      if (apiResponse.ok) {
        ...
        console.error(
          "[UserContext logout] API logout call failed:",
          apiResponse.status,
      ...
      console.error(
        "[UserContext logout CATCH_ERROR] Error calling API to invalidate server session:",
        err
      );
    } finally {
      setUser(null);
      ...
      setInitialAuthChecked(true);
      console.log(
        "[UserContext logout FINALLY] Client state cleared. isLoading: false, initialAuthChecked: true. Redirecting to login."
      );
  ...
  const switchPractice = async (newPracticeId: string) => {
    if (
      ...
      try {
        ...
        setIsLoading(false);
        console.log(
          "[UserContext switchPractice FINALLY] isLoading set to false."
        );
      }
    } else {
      console.warn(
        "[UserContext switchPractice] Switch practice called by non-admin or no user."
      );
    }
  };
  ...
  if (!initialAuthChecked) {
    console.log(
      "[UserProvider Render] Initial auth not checked. Returning null."
    );
    return null;
  }
...
export function useUser() {
  ...
  const userPracticeId = context.user
    ? context.user.role === "CLIENT" ||
      context.user.role === "PRACTICE_ADMINISTRATOR" ||
      context.user.role === "VETERINARIAN" ||
      context.user.role === "PRACTICE_MANAGER"
      ? context.user.practiceId &&
        context.user.practiceId.toString().trim() !== ""
        ? context.user.practiceId
        : undefined
      : context.user.role === "ADMINISTRATOR" ||
        context.user.role === "SUPER_ADMIN"
      ? context.user.currentPracticeId &&
        context.user.currentPracticeId.toString().trim() !== ""
        ? context.user.currentPracticeId
        : undefined
      : undefined
    : undefined;
  ...
  console.log(
    "[useUser] Raw practiceId:",
    context.user && "practiceId" in context.user
      ? context.user.practiceId
      : "N/A"
  );
  console.log(
    "[useUser] Raw currentPracticeId:",
    context.user && "currentPracticeId" in context.user
      ? context.user.currentPracticeId
      : "N/A"
  );
  ...
  const getUserPracticeId = useCallback(() => {
    ...
    if (
      ...
      return practiceId && practiceId.toString().trim() !== ""
        ? practiceId
        : undefined;
    }
    ...
    if (
      ...
      return currentPracticeId && currentPracticeId.toString().trim() !== ""
        ? currentPracticeId
        : undefined;
    }
